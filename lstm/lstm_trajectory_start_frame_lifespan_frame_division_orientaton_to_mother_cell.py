...
########################################## lstm use trajectory, start_frame, lifespan_frame and division_orientation_to_mother_cell features ###########################################################################################
cv_train_loss = []
cv_train_accuracy = []
cv_val_loss = []
cv_val_accuracy = []
...
for i in range(6):
    ...
    for epoch in range(1,num_epochs+1):
        ...
        # if epoch%save_interval == 0: # save test model
            # torch.save(model.state_dict(), f"./model_pt/lstm_traj_sf_lf_dm_{i}_fold_CV_{epoch}_wd02.pt")
...
with open( './cross_validation/lstm_traj_sf_lf_dm_cv_train_loss_wd02.json', 'w') as f:
   json.dump(cv_train_loss, f)
with open( './cross_validation/lstm_traj_sf_lf_dm_cv_val_loss_wd02.json', 'w') as f:
   json.dump(cv_val_loss, f)
with open( './cross_validation/lstm_traj_sf_lf_dm_cv_train_accuracy_wd02.json', 'w') as f:
   json.dump(cv_train_accuracy, f)
with open( './cross_validation/lstm_traj_sf_lf_dm_cv_val_accuracy_wd02.json', 'w') as f:
   json.dump(cv_val_accuracy, f)
with open( './cross_validation/lstm_traj_sf_lf_dm_cv_test_accuracy_wd02.json', 'w') as f:
...
for epoch in range(1,num_epochs+1):
    ...
    if epoch%save_interval == 0: # save model test
        torch.save(model.state_dict(), f"./model_pt/lstm_traj_sf_lf_dm_test_{epoch}_wd02.pt") 
# Test
acc_test, _ = _utilities.evaluate_rnn(model, test_dl, optimizer, device)
# test_accuracy.append(acc_test)
print(f'Final Test_accuracy: {acc_test:.4f}')

with open( './test/lstm_traj_sf_lf_dm_test_train_loss_wd02.json', 'w') as f:
   json.dump(lstm_train_loss, f)
with open( './test/lstm_traj_sf_lf_dm_test_train_accuracy_wd02.json', 'w') as f:
   json.dump(lstm_train_accuracy, f)
with open( './test/lstm_traj_sf_lf_dm_test_test_accuracy_wd02.json', 'w') as f:
