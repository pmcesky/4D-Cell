...
for i in range(6):
    ...
    for epoch in range(1,num_epochs+1):
        ...
        if epoch%save_interval == 0: # save test model
            torch.save(model.state_dict(), f"./mlp/model_pt/mlp_traj_{i}_fold_CV_{epoch}.pt")
...
with open( './mlp/mlp_traj_cv_train_loss.json', 'w') as f:
   json.dump(cv_train_loss, f)
with open( './mlp/mlp_traj_cv_val_loss.json', 'w') as f:
   json.dump(cv_val_loss, f)
with open( './mlp/mlp_traj_cv_train_accuracy.json', 'w') as f:
   json.dump(cv_train_accuracy, f)
with open( './mlp/mlp_traj_cv_val_accuracy.json', 'w') as f:
   json.dump(cv_val_accuracy, f)
with open( './mlp/mlp_traj_cv_test_accuracy.json', 'w') as f:
...
for epoch in range(1,num_epochs+1):
    ...
    if epoch%save_interval == 0: # save model test
        torch.save(model.state_dict(), f"./mlp/model_pt/mlp_traj_test_{epoch}.pt") 
# Test
acc_test, _ = _utilities.evaluate_mlp(model, test_dl, optimizer, device)
# test_accuracy.append(acc_test)
print(f'Final Test_accuracy: {acc_test:.4f}')

with open( './mlp/mlp_traj_test_train_loss.json', 'w') as f:
   json.dump(mlp_train_loss, f)
with open( './mlp/mlp_traj_test_train_accuracy.json', 'w') as f:
   json.dump(mlp_train_accuracy, f)
with open( './mlp/mlp_traj_test_test_accuracy.json', 'w') as f:
